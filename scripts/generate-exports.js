...
function getFiles(dir, allFiles = []) {
  ...
  files.forEach((file) => {
    ...
    if (fs.statSync(filePath).isDirectory()) {
    ...
    } else if ((file.endsWith('.ts') || file.endsWith('tsx')) && file !== 'index.ts' && !file.startsWith('_')) {
      ...
      // Only replace .ts if it's NOT followed by 'x'
      // \.ts(?!x) matches ".ts" but only if the next character isn't "x"
      relativePath = relativePath.replace(/\.ts(?!x)$/, '.js');
