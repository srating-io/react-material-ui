  ...
  if (customDecorateRows) {
    ...
    row_containers = visibleRows.map((row, index) => {
      ...
      for (let i = 0; i < displayColumns.length; i++) {
        ...
        if (secondaryKey && rowData[`${displayColumns[i]}_${secondaryKey}`]) {
          secondarySpan = (
            <span style = {{ color: theme.grey[500] }}> {rowData[`${displayColumns[i]}_${secondaryKey}`] as string}</span>
          );
        }

        tableCells.push(
          <Td key = {i} style = {cellStyle}>{rowData[displayColumns[i]] as string || defaultEmpty}{secondarySpan}</Td>,
        );
      }
